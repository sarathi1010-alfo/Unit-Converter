...
export default async function CategoryConversionPairPage(
  ...
  const conversionFactor = categoryId === 'temperature' || categoryId === 'clothing'
    ? 'non-linear'
    : fromUnit.baseFactor / toUnit.baseFactor;
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer: conversionFactor === 'non-linear'
        ? `The relationship between ${fromUnit.name} and ${toUnit.name} is non-linear. Use our calculator for an accurate conversion.`
        : `There are exactly ${conversionFactor} ${toUnit.name} in one ${fromUnit.name}.`
    },
    {
